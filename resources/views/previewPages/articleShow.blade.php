...
$parts = DB::table('makalat_3ama')
->select('*', DB::raw('COALESCE((SELECT COUNT(*) FROM likes WHERE likes.contents_id = makalat_3ama.id), 0) as likes_count'))
    ->get();
 ...
 $likes_count = DB::table('likes')
        ...
        ->where('branch_id', $article->branch_id)
        ->where('item_id', $article->item_id)
        ->count();

$latestTitles = DB::table('makalat_3ama')
...
<header class="HeaderHome  top-0 left-0 right-0 p-4">
   ...
   background-repeat: no-repeat; background-position: center center; " >
        <div class="w-[65%]">
          <h1 class=" text-slate-300 font-bold mb-4 text-[3.5rem]" style="font-family:'zain'; direction:rtl;">{{ $article->title }}</h1>
        </div>
        <div class="relative ">
    ...
    <h2 class="relative z-10 flex flex-col items-center justify-center w-full h-full text-center  p-1"  style="direction:rtl">
        @php
            $length = strlen($article->topic_no);
            $fontSize = $length > 3 ? ($length > 4 ? '2rem' : '2.5rem') : '4rem';
        @endphp
        <span style="font-size: {{ $fontSize }}; font-family:'sakkal majalla'; line-height: 1;">{{ $article->topic_no }}</span>
        <div class="text-[2rem]">{{ $article->topic_letter }}</div>
    </h2>
</div>
        </div>
...
<div id="article-content" class="container w-[60vw] ml-[25vw]  py-[2rem] relative mb-[42rem]">
    ...
    <h1 class="text-4xl text-slate-200 font-bold mb-4 leading-relaxed underline underline-offset-8" style="font-family: 'zain'; "><span class="text-[#FAE1C6]">{{ $article->topic_no }} </span><span class="text-[#FAE1C6]">{{ $article->topic_letter }}: </span > {{ $article->title }} </h1>
    <div class="justify-start items-center gap-[5px] mt-5 inline-flex w-[100%]" style="font-family:'El messiri'; ">
            ...
            <div class="w-[40%]  text-[1rem] font-normal text-right leading-[18px]" style="font-family:'Noto Kufi Arabic'; color:#C18F59;">{{ $article->author }} </div>
            
            
            <div class=" 2xl:text-[1rem] lg:text-[0.9rem] inline-flex items-center gap-2" style="font-family:'Noto Kufi Arabic';"><span class="date-icon w-5 h-5"><img src="{{ asset('images/date.png')}}" alt="date-icon"></span>بتاريخ: <span class="text-[#C18F59]">{{ $article->day}} / </span><span class="text-[#C18F59]">{{$article->month}} / </span><span class="text-[#C18F59]"> {{ $article->year}}</span></div>
    </div>
   
    

  
    <h2 class="text-3xl font-bold my-5 border-b border-[#A6743E] py-5" style=" !important; font-family:'El Messiri'; text-align: center;">{!! $article->mola5s !!}</h2>
<!--audio player-->

    {{-- @if($article->audio_files)
        <div class="audio-player">
            <audio controls>
                <source src="{{ asset('storage/' . $article->audio_files) }}" type="audio/mpeg">
...
<!--audio player end-->
    <h2 id="ka3da" class="text-3xl font-bold my-5 " style="color:#fdba74 !important; font-family:'El Messiri'; text-align: center;">{{ $article->title }}</h2>
    ...
    <div class="my-5 border-b border-[#A6743E] pb-8">{!! $article->article_text !!}</div>
    
    
   
</div>
<!--add comment section-->
<div id="commentSection" class="my-8  pt-8 z-[200]">
    <div class="flex justify-end ">
        ...
        @csrf
        <input type="hidden" name="contents_id" value="{{ $article->id }}">
    ...
    <div  class="action-buttons lg:top-[28rem] 2xl:top-[28.5rem] top-[18rem] 2xl:right-[6.2rem] lg:right-[4rem] right-[2rem] absolute">
        <div class="w-[5.7rem] h-auto border-[1px] border-white text-white text-center ">
            <div class="border-b-2 border-white h-[5rem] flex items-center justify-center">
                ...
                onclick="likeArticle({{ $article->id }}, {{ $article->section_id }}, {{ $article->branch_id }}, {{ $article->item_id }})">
                    <img id="like-icon-{{ $article->id }}" src="{{ asset('images/like.png') }}" alt="like-icon" style="transition: all 0.3s ease;">
                    <span id="like-count-{{ $article->id }}" class="text-white ml-2">{{ $likes_count}}</span>
                </div>
            </div>
            <div class="border-b-2 cursor-pointer  border-white h-[5rem] flex items-center justify-center">
                <div class="favorite w-[100%] h-24 flex justify-center items-center" onclick="saveToFavorites({{ $article->id }})">
            ...
            <div id="download_pdf" class="h-[5rem] flex items-center justify-center">
                <div class="Frame34 flex justify-center gap-[3px] ">
                @if (!empty($article->pdf_files))<!-- Ensure $record->pdf_file contains the PDF path -->
                <a href="{{ asset('storage/pdf_files/' . basename($article->pdf_files)) }}" 
                    download="{{ $article->title ?? 'file' }}.pdf" class="btn btn-primary">
